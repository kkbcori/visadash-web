...
import { runAudit, nameOutcome, parseI94, fieldsFromPaste } from "../src/engine/audit.mjs";
...
test("clean DS-160 vs matching passport → no blockers + verified matches", () => {
  ...
  assert.ok(r.counts.match >= 3, "expected verified matches, got " + r.counts.match);
});
...
test("passport-number O/0 confusion is flagged as a blocker with a specific message", () => {
  const passZero = "Z0234567<8IND9001011F3001012<<<<<<<<<<<<<<04";
  const ds = ds160(DS_OK.replace("Z1234567", "ZO234567"));
...
test("passport expiring within 6 months is a warning", () => {
  const soon = "Z1234567<8IND9001011F2611012<<<<<<<<<<<<<<04";
...
test("pasted personal details mismatch is a blocker", () => {
  const r = runAudit({
    ds160: ds160(DS_OK),
    personal: fieldsFromPaste({ surname: "SHARNA", given: "PRIYA", dob: "01-JAN-1990" }),
  }, { now: NOW });
  assert.ok(r.findings.some(f => f.id === "personal-surname" && f.severity === "blocker"));
});

test("pasted address match is verified", () => {
  const ds = ds160(DS_OK + "\nStreet Address (Line 1): 12 MG ROAD\nCity: BENGALURU\nE-mail Address: PRIYA@EXAMPLE.COM");
  const r = runAudit({
    ds160: ds,
    address: fieldsFromPaste({ homeAddress: "12 MG ROAD", homeCity: "BENGALURU", email: "priya@example.com" }),
  }, { now: NOW });
  assert.ok(r.matches.some(m => m.id === "addr-city" || /city/i.test(m.message)));
  assert.equal(r.counts.blocker, 0, JSON.stringify(r.findings));
});
