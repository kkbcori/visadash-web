...
// Strip leading separators, "(n)" index tokens ("Label: (1): VALUE") and a leading
// parenthetical qualifier that sits between the label and the value
// ("Monthly Salary in Local Currency (if employed): 7500").
const stripSep = s => s.replace(/^(?:\s|[:.\-–—>|]|\([^)]*\))+/, "").trim();
...
const DS160_FIELDS = [
  ...
  { key: "permanentResidentCountry", label: "Other permanent-resident country", cat: "Personal Information 2", n: norm.upper, s: "same", sv: "low", labels: ["Other Permanent Resident Country/Region", "Other Permanent Resident Country"] },
