...
function paintComparison(o) {
  ...
  const content = o.contentHtml != null ? o.contentHtml
    : `${o.changedRows
       ...
       ${o.sameRows
        ? `<details class="cmp-collapse" open><summary>${o.sameCount} unchanged item${o.sameCount === 1 ? "" : "s"}</summary>${cmpTable(o.colA, o.colB, o.sameRows)}</details>` : ""}`;
...
function renderEngineResult(eng) {
  ...
  const groupsHtml = cats.map(cat => {
    const g = groups.get(cat);
    const open = " open";   // all sections expanded by default (Collapse all still available)
